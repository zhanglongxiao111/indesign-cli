...
- 不把客户文档内容、客户名称或私有资产完整路径写入日志和 session。
...
3. **Operation Affordance**：这个对象适合执行什么操作。包括 `frame.move`、`image.pan_content`、`story.replace_text`、`style.apply_paragraph_style`、`master.override_item` 等。
...
```json
{
  ...
  "semantic": {
    "role": "graphic_frame",
    "roleSource": "contains placed graphic",
    "confidence": 1,
...
也就是说，CLI 的 `schema_version` 表示 CLI envelope 版本，`semanticContractVersion` 表示 InDesign 语义 payload 版本。两者必须分别演进。

## 语义层模块架构

语义提升不应靠在每个 tool description 里手动复制一大段说明，也不应维护一个超大静态字典。正确架构是建立共享语义层：少量静态注册表 + ExtendScript 运行时推断 + Node 侧 normalizer + 统一 contract。
...
- **Host side**：注册表、JSON schema、工具说明生成、warning schema、脱敏、结果 normalizer。
...
```text
src/semantics/
  registry.js          # 静态语义注册表：原生类型、role、说明、推荐操作
  ...
  redaction.js         # 路径、文档名、标签、文本预览脱敏
  normalizers.js       # Node 侧清洗 ExtendScript 返回，补 envelope、alias、默认值
  toolDescriptions.js  # 从 registry 生成或复用工具说明片段
...
| 文件 | 职责 |
| --- | --- |
| `registry.js` | 手写少量 InDesign 原生类型与语义角色映射，例如 `Rectangle`、`Image`、`TextFrame`、`Story`、`ParagraphStyle` |
...
| `normalizers.js` | Node 侧解析、脱敏、补默认字段、把旧 alias 转成新 contract |
| `redaction.js` | 统一处理 `documentName`、路径、label、文本预览和 session/log 输出 |
| `warnings.js` | 根据对象角色和操作意图生成统一 warning |
| `toolDescriptions.js` | 从 registry/contract 生成工具说明片段，避免文档漂移 |

这层应遵守：

- 静态表只记录稳定知识，不记录具体文档对象。
- 动态推断必须来自 InDesign DOM，不凭名称硬猜。
- 工具说明、warning、`operationHint` 和 `affordances` 尽量从同一注册表派生。
...
本项目不能记录客户文档内容、客户名称或私有资产完整路径。语义 contract 必须字段级约束，而不是只写原则。
...
```json
{
  ...
  "extension": ".png",
  "pathKnown": true,
  "pathScope": "local_or_network",
  "pathHash": "sha256:...",
  "redactedPath": "<redacted>/plan.png"
}
```

规则：

- 默认不返回完整 `filePath`、`bookPath`、`outputPath`。
- `documentName`、`label`、`page item name` 也可能含客户信息，进入 session/log 前必须脱敏或哈希。
- `contentsPreview` 默认限长，且不得写入 session。
- CLI stderr、session 和错误详情都必须走同一 `redaction.js`。
- 只有用户显式请求调试路径时，才允许在当前终端返回完整路径；仍不写入长期日志。
...
1. `objectId` + scope 校验。
2. `script label` / `label`。
3. `displayPageLabel` 或 `physicalPageNumber` + `objectId`。
4. `pageIndex` + `itemIndex`。
5. `semantic.role` + style/name/layer/link/bounds proximity。
...
- `documentFingerprint`：文档名称脱敏值、保存路径哈希、页数、修改状态摘要。
...
CLI envelope 的顶层 `warnings` 和工具 payload 内的 `warnings` 必须合并去重。去重 key 为 `code + target.native.objectId + operation`。CLI 可额外增加 transport/runtime warning，但不能丢失工具 warning。
...
| Links / Placed Assets | core v1 | `link`、`assetKind`、`linkStatus`、`parentFrame` | `LINK_PATH_REDACTED`、`LINK_MISSING` | fixture + 真实 E2E |
...
`known-deferred` 表示语义注册表先占位并记录边界，第一版可只读或不暴露写工具。`unsupported` 只能用于明确不打算支持的能力，并必须写明替代路径。

## 推荐实施顺序

1. 新增 `src/semantics/contract.js`、`schemas.js`、`registry.js`，先固定 `semanticContractVersion`、payload schema、warning schema。
2. 建立 `tests/fixtures/semantics/` golden fixtures 和 contract tests，先测 schema、normalizer、redaction、warning 合并。
3. 新增 `src/semantics/redaction.js`、`warnings.js`、`normalizers.js`，并接入 CLI envelope 的解析路径。
4. 新增 `src/semantics/jsxRuntime.js`，把 ExtendScript DOM 推断、父子关系、bounds 序列化集中成共享 snippet。
5. 增强 `page.list_page_items` 和 `get_page_item_info` 为结构化 inspector，并支持 `verbosity/include/maxItems/pageSize/cursor`。
6. 新增 `selector.query_items`，先解决“找对象”和“重验证”。
7. 给现有通用写工具加 warning、`semanticContractVersion`、`targetWasExplicit` 和 before/after diff。
8. 新增 `frame.*`、`image.*`、`story.*`、`style.*`、`link.*` 等窄语义工具。
9. 更新 CLI 文档、Agent Skill、MCP prompt，让说明复用 `src/semantics/` 的注册表。
10. 每阶段运行 schema tests、CLI 单元测试；触及真实 InDesign 行为时补真实 E2E 和性能/隐私验收。
