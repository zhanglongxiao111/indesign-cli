...
indesign-cli tool search --domain export --query pdf
```
...
indesign-cli tool domains
indesign-cli tool list --domain template
indesign-cli tool list --domain export --source classic
indesign-cli tool search --domain document --query "active document"
indesign-cli tool schema document.get_info
indesign-cli tool call document.get_info --args args.json
```
...
```powershell
indesign-cli script run path/to/script.jsx
Get-Content path/to/script.jsx | indesign-cli script run --stdin
```
...
indesign-cli session show --verbose
indesign-cli session clear
```
...
indesign-cli export verify path/to/output.idml --created-after 2026-05-23T15:00:00+08:00
```
