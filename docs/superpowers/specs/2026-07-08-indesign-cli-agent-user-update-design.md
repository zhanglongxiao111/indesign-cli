...
第一版必须注册当前用户级命令入口，让 Agent 能直接调用：

```powershell
indesign-cli-agent
```

注册方式可以是把 `%LOCALAPPDATA%\indesign-cli\bin` 写入当前用户 PATH，或提供等价的当前用户级命令入口；不得写系统 PATH、HKLM 注册表或要求管理员权限。安装后的新 Agent 线程不应再寻找 EXE 绝对路径。首次安装脚本可以在安装当次使用绝对路径完成引导，但安装完成后的标准入口必须是命令名。
...
- `sha256` 必须校验。
- 新发布的 `latest.json` 只承诺新 `artifact.*` schema，不为旧 `run --source` schema 保留顶层 `url` / `sha256` 兼容字段。
...
- Agent 标准入口是用户级命令 `indesign-cli-agent`。
- Agent 不应在每个新线程里重新查找 EXE 绝对路径；命令注册失败才允许使用 `%LOCALAPPDATA%\indesign-cli\bin\indesign-cli-agent.exe` 作为修复线索。
- 正式调用前必须执行更新检查。
...
- README / Skill 的成品 EXE 标准入口改为用户级命令 `indesign-cli-agent`。
- 旧 `run --source` 成品入口直接废弃，不做短期兼容保留。
- 实施后 `run --source` 不应继续作为公开命令存在；如果代码内部仍临时保留，必须隐藏、标记废弃，并不得出现在 README、Skill、帮助输出或发布契约中。
- 旧 `%LOCALAPPDATA%\indesign-cli\current/runtime` 目录不再是成品 EXE 更新的权威状态。
- 新发布的 `latest.json` 不为旧 `run --source` 保留 schema 兼容。
- 实施计划必须停止旧 runtime 缓存写入，并清理或忽略旧 `%LOCALAPPDATA%\indesign-cli\current/runtime` 状态；新链路不能继续产生多版本 runtime 垃圾。
...
- 首次安装后，新的 Agent 线程可以直接调用 `indesign-cli-agent`，不需要重新查找 EXE 路径。
- 已安装机器每次正式调用前会检查版本。
- NAS 可用时优先走 NAS。
- NAS 不可用时自动走 GitHub。
...
- 旧 `run --source` 成品入口直接废弃，发布契约不再兼容它。
- 更新流程不在项目目录产生文件。
