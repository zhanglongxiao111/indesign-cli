...
**Architecture:** 遥测和 `feedback` 域全部在 Python CLI 侧（`cli.primitive` overlay，不进 Node registry）；事件白名单 schema、每次调用直接写入 NAS 收集根目录，按 `origin_key + session_id` 分文件；会话级指标与重试链由分析端从事件流推导，CLI 侧不计算；聚合是确定性脚本，LLM 只做周报表述和修复建议。
...
- 公司默认收集根目录为 `\\daga-nas5\sa-ai-app\feedback-reports\indesign-cli-telemetry`；CLI 直接写 NAS，不做本地中转上传。
- SA AI APP / OpenCode 的精确线程绑定由上层运行时后续注入 `INDESIGN_CLI_AGENT_THREAD_ID` / `INDESIGN_CLI_AGENT_RUN_ID`，本仓库先实现 env 接口和默认分组规则。
...
- [ ] `telemetry.py` 提供 `record_event(event_dict)`：按白名单 schema 过滤字段（未知字段丢弃）；`INDESIGN_CLI_TELEMETRY_DIR` 配置时直接 append 到 NAS `<root>/sessions/YYYY-MM-DD/<origin_key>__<session_id>.jsonl`；任何写失败静默降级，绝不抛错影响命令结果。
- [ ] `telemetry.py` 同时维护 NAS `<root>/state/<origin_key>.json`，字段只含 `session_id`、`created_at`、`last_seen_at`、`cwd_hash`；不保存完整 cwd。`origin_key = hash(COMPUTERNAME, USERNAME, cwd_hash)`。
- [ ] session 生成优先级：`INDESIGN_CLI_SESSION_ID` 显式覆盖；否则 `INDESIGN_CLI_AGENT_THREAD_ID` + `INDESIGN_CLI_AGENT_RUN_ID` 构造；否则按 `origin_key` 复用 state，空闲超过 8 小时生成新 session。空闲阈值可由 `INDESIGN_CLI_TELEMETRY_IDLE_HOURS` 覆盖。
- [ ] 事件字段（白名单全集）：`ts`、`session_id`、`origin_key`、`cwd_hash`、`agent_thread_id`、`agent_run_id`、`cli_version`、`registry_hash`（当前缺省，终态重构落地后填充）、`event`、`tool_id`、`source`、`ok`、`error_code`、`duration_ms`、`arg_keys`、`via_batch`、`code`、`note`、`recent_calls`。
- [ ] `INDESIGN_CLI_TELEMETRY=off` 完全禁用；未配置 `INDESIGN_CLI_TELEMETRY_DIR` 时不写共享遥测，只保留现有 CLI session 功能。
...
- [ ] `test_feedback.py` 覆盖：白名单过滤（塞入非法字段被丢弃）、off 开关、NAS `sessions/YYYY-MM-DD` 与 `state` 目录写入、收集目录不可写时静默降级、idle timeout 自动切新 session、`INDESIGN_CLI_SESSION_ID` 显式覆盖、`INDESIGN_CLI_AGENT_THREAD_ID` / `INDESIGN_CLI_AGENT_RUN_ID` 注入、batch 子调用事件、`arg_keys` 无参数值、事件不含完整 cwd。
...
- [ ] 手动冒烟：设置 `INDESIGN_CLI_TELEMETRY_DIR=\\daga-nas5\sa-ai-app\feedback-reports\indesign-cli-telemetry` 后运行 `indesign-cli server health`，确认 NAS `sessions/YYYY-MM-DD/*.jsonl` 与 `state/*.json` 出现且无参数值、无完整路径。
...
- [ ] `indesign_cli.py` 新增 `feedback report` 子命令；执行时记一条 `feedback` 遥测事件，自动附加 `session_id`、`origin_key`、`cwd_hash`、`cli_version`、`agent_thread_id` / `agent_run_id`（如有）、最近 5 条 `tool_call` 事件摘要（`recent_calls`，脱敏形态：tool_id + ok + error_code）。
...
- [ ] README / README.en 增加 `feedback` 域用法、遥测说明（记录什么、不记录什么）、`INDESIGN_CLI_TELEMETRY` / `INDESIGN_CLI_TELEMETRY_DIR` / `INDESIGN_CLI_SESSION_ID` / `INDESIGN_CLI_AGENT_THREAD_ID` / `INDESIGN_CLI_AGENT_RUN_ID` / `INDESIGN_CLI_TELEMETRY_IDLE_HOURS` 配置；默认示例路径写 `\\daga-nas5\sa-ai-app\feedback-reports\indesign-cli-telemetry`。
- [ ] `AGENTS.md` §2.5 增加长期约束：遥测字段白名单制，禁止记录参数值、完整路径与客户信息；新增字段必须过 review；共享遥测目录只写 NAS JSONL / state / reports，不写客户素材。
- [ ] 确认 `skills/indesign-cli/preview.png` 是否仍匹配更新后的 Skill 说明。
- [ ] **批 1 合并检查点**：本任务完成并合并后，才允许终态重构执行 Task 0 golden master 录制。

**Verify:**

- [ ] `rg -n "feedback report|TOOL_GAP|INDESIGN_CLI_TELEMETRY|INDESIGN_CLI_AGENT_THREAD_ID|daga-nas5" skills/indesign-cli/SKILL.md README.md README.en.md AGENTS.md`
- [ ] `git diff --check`
- [ ] `python -m pytest agent-harness\cli_anything\indesign\tests -q`
...
- [ ] `aggregate.py --input <遥测根目录或 sessions 目录> --output <聚合JSON路径>`：递归读取 `sessions/**/*.jsonl`，丢弃未知字段和坏行（计数报告，不中断）。
...
- [ ] 会话来源分布只输出 `origin_key` / `cwd_hash` 数量分布，不含完整路径或原始用户信息。
