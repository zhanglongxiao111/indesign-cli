...
| Task | 状态 | 负责人 | 更新时间 | 备注 |
| ---- | ---- | ------ | -------- | ---- |
| Task 0 冻结、快照与基线导出 | spec_review | implementation subagent (`gpt-5.5 high`) / review subagent (`gpt-5.4 xhigh`) | 2026-07-07 | D raw evidence 修复已提交 `5c3f7bc`：`record_golden` 校验并落盘 fresh raw catalog evidence，D total=150 且含 `feedback.report`；等待复审 |
