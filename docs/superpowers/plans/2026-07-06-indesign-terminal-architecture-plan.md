...
| Task 2 15 个域并行迁移 | in_progress | per-domain implementation subagents (`gpt-5.5 high`) | 2026-07-07 | 12 域已完成：第一批 6 域 + `spread`、`pageItem`、`group`、`book`、`presentation`、`export`；第三批 `utility`、`help`、`template` 待启动，域 agent 只写本域 `src/tools/<domain>/`，全局聚合由 controller 串行接入 |
