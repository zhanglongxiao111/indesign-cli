...
| Task 2 15 个域并行迁移 | in_progress | per-domain implementation subagents (`gpt-5.5 high`) | 2026-07-07 | 第一批 6 域已完成并推送；第二批 `spread`、`pageItem`、`group`、`book`、`presentation`、`export` 进行中；`utility`、`help`、`template` 排队，域 agent 只写本域 `src/tools/<domain>/`，全局聚合由 controller 串行接入 |
