...
| Task | 状态 | 负责人 | 更新时间 | 备注 |
| ---- | ---- | ------ | -------- | ---- |
| Task 0 冻结、快照与基线导出 | spec_fixing | implementation subagent (`gpt-5.5 high`) / review subagent (`gpt-5.4 xhigh`) | 2026-07-07 | spec review 未通过：`contract_baseline.py` 复制 CLI contract 推断逻辑，违反 SSOT；需收回到 `catalog.py` canonical inference 并重导 baseline |
