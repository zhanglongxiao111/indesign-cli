...
| Task | 状态 | 负责人 | 更新时间 | 备注 |
| ---- | ---- | ------ | -------- | ---- |
| Task 0 冻结、快照与基线导出 | spec_review | implementation subagent (`gpt-5.5 high`) / review subagent (`gpt-5.4 xhigh`) | 2026-07-07 | spec baseline 修复已提交 `881ed01`：`destructive` 收口到 `catalog.py` helper，D inventory=150 且含 `cli.primitive/feedback.report`；等待复审 |
